...
/*   Updated: 2021/02/16 13:11:40 by mmonroy-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			ft_get_fork(unsigned long id,
					struct timeval *time, t_sem *data)
{
	sem_wait(data->s_forks);
	gettimeofday(time + 2, 0);
	ft_print_change(ft_get_time(time, time + 2),
	id, "has taken a fork\n", data);
	sem_wait(data->s_forks);
	gettimeofday(time + 2, 0);
	ft_print_change(ft_get_time(time, time + 2),
...
static void			ft_philosophize(unsigned long id,
					struct timeval *time, int *meals, t_sem *data)
{
