...
/*   Updated: 2021/02/16 12:52:10 by mmonroy-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_create_threads(pthread_t *phi_threads, unsigned long *phi_id)
{
	...
	if (!(g_s_forks = sem_open("forks", O_CREAT, 0664, g_arg.num_phi))
			|| !(g_s_stdout = sem_open("stdout", O_CREAT, 0664, 1))
			|| !(g_s_meals = sem_open("meals", O_CREAT, 0664, 1))
			|| !(g_s_time = sem_open("time", O_CREAT, 0664, 1)))
		return (1);
	i = -1;
